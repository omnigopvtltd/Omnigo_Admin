...
import {
  useRestaurants, useCreateRestaurant, useUpdateRestaurant,
  useUpdateRestaurantStatus, useDeleteRestaurant,
...
export default function RestaurantsPage() {
  ...
  const { data, isLoading, isFetching } = useRestaurants({ status, search, page, limit: 8 });
  const restaurants = data?.restaurants ?? [];
  

  const createMutation = useCreateRestaurant();
  const updateMutation = useUpdateRestaurant();
  const statusMutation = useUpdateRestaurantStatus();
  ...
  const table = useReactTable({ data: restaurants, columns, getCoreRowModel: getCoreRowModel() });
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-1 py-16 text-center">
              <p className="text-sm font-medium text-foreground">No restaurants found</p>
