...
function nextId(prefix) {
  return (
    prefix + Math.random().toString(16).slice(2).padEnd(20, "0").slice(0, 20)
  );
}
...
export async function getSessions({ isActive, page = 1, limit = 10 } = {}) {
  // Real version:
  const { data } = await axiosClient.get("/riders/sessions/", {
...
export async function createSession(payload) {
  // Real version:
  const { data } = await axiosClient.post("/riders/sessions/create", payload);
  return data.session;
  // const session = {
  //   _id: nextId("session"),
...
export async function deleteSession(id) {
  // Real version:
  await axiosClient.delete(`/riders/sessions/delete/${id}`);
  return;
  // const hasActive = participations.some(
  //   (p) => p.sessionId._id === id && p.status === "in_progress",
  // );
  // if (hasActive)
  //   throw new Error("Can't delete a session with riders currently in progress");
...
export async function getSessionParticipants(id, { status } = {}) {
  ...
  const { data } = await axiosClient.get(
    `/riders/sessions/${id}/participants`,
    { params: { status } },
  );
